...
esp_err_t esp_rmaker_time_sync_init(esp_rmaker_time_config_t *config)
{
    ...
    char *esp_sntp_server_name;
    if (!config || !config->sntp_server_name) {
        esp_sntp_server_name = CONFIG_ESP_RMAKER_SNTP_SERVER_NAME;
    } else {
        esp_sntp_server_name = config->sntp_server_name;
    }
    ESP_LOGI(TAG, "Initializing SNTP. Using the SNTP server: %s", esp_sntp_server_name);
    esp_sntp_setoperatingmode(SNTP_OPMODE_POLL);
    esp_sntp_setservername(0, esp_sntp_server_name);
    esp_sntp_init();
    if (config && config->sync_time_cb) {
        esp_sntp_set_time_sync_notification_cb(config->sync_time_cb);
    } else {
        esp_sntp_set_time_sync_notification_cb(esp_rmaker_time_sync_cb);
    }
